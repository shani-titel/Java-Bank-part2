...
public class ServiceAndCustomerWindow extends Application implements
	...
	/*
	 * public ServiceAndCustomerWindow(Controller controller) { super();
	...
	public void start(Stage primaryStage, BankActions chosenAction,
			Client[] clients, Atm[] atms, Banker[] bankers) throws Exception {
	...
	public void startView(Stage primaryStage, final BankActions chosenAction,
			Client[] clients, Atm[] atms, Banker[] bankers) {
		if (chosenAction != null && clients != null && atms != null
				&& bankers != null) { // check
			// build grid
			...
			customersList.getSelectionModel().selectedItemProperty()
			...
			bankersList.getSelectionModel().selectedItemProperty()
					.addListener(new ChangeListener<String>() {
						@Override
						public void changed(ObservableValue observable,
								String oldValue, String newValue) {
			...
			ATMsList.getSelectionModel().selectedItemProperty()
					.addListener(new ChangeListener<String>() {
						@Override
						public void changed(ObservableValue observable,
								String oldValue, String newValue) {
			...
			NextBtn.setOnAction(new EventHandler<ActionEvent>() {
				...
				public void handle(ActionEvent event) {
					if (isBankerSelected == true && isATMSelected == true) {
						...
						try {
							msgbox.start(anotherStage,
									"You can choose banker OR atm!");
						} catch (Exception e) {
							e.printStackTrace();
						}
					} else if (isClientSelected == true) {
						if (isBankerSelected == true) {
							// BankActionsWindow bankeActions = new
							// BankActionsWindow();
							try {
								SpecificActionWindow actionApp = new SpecificActionWindow();
								Controller controller1 = new Controller(model,
										actionApp); // create new controller
								actionApp.start(new Stage(), chosenAction,
										customersList.getSelectionModel()
												.getSelectedItem(),
										EMPTY_STRING, bankersList
												.getSelectionModel()
												.getSelectedItem());
							} catch (Exception e) {
								e.printStackTrace();
						...
						else if( chosenAction == BankActions.BankCharge || chosenAction == BankActions.BankCredit){
							// open small action window
							SpecificActionWindow actionApp = new SpecificActionWindow();
							Controller controller1 = new Controller(model,
									actionApp); // create new controller
							try {
								actionApp.start(new Stage(), chosenAction,
										customersList.getSelectionModel().getSelectedItem(),EMPTY_STRING ,EMPTY_STRING);
							} catch (Exception e) {
						...
						try {
							msgbox.start(anotherStage,
									"You have to choose banker OR atm AND a client!");
						} catch (Exception e) {
							e.printStackTrace();
						}
					}
			...
			// add lists to grid
			if (chosenAction == BankActions.BankCharge
					|| chosenAction == BankActions.BankCredit) {
				
				ATMsList.setDisable(true);
				bankersList.setDisable(true);
			}
