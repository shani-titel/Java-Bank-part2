...
public class SpecificActionWindow extends Application implements ApplicationBase{
	...
	public SpecificActionWindow() {
		super();
	}
/*
	public SpecificActionWindow(Controller controller) {
		super();
	...
	private void startView(final Stage primaryStage, final BankActions chosenAction, String clientName, String atmLocation, String bankerName) throws Exception {
		...
		if(chosenAction!=null && clientName!=null && atmLocation!=null && bankerName!=null){ //check
			...
			else if(!atmLocation.isEmpty()){
				commandText = new Text("Bank Actions for Client: " + clientName + " | ATM: " + atmLocation);
			}
			else{
				commandText = new Text("Bank Actions for Client: " + clientName);
			}
	            ...
	            public void handle(ActionEvent event) {
	                if(!textField1.getText().isEmpty()){
	                	if(chosenAction == BankActions.Deposit){
	        				...
	        				depositInUI((int)client.getClientId(), Double.parseDouble(textField1.getText()), serviceGiver);
	        			}
	        			else if(chosenAction == BankActions.Withdraw){
	        				whithdrawInUI((int)client.getClientId(), Double.parseDouble(textField1.getText()), serviceGiver);
	        			}
	        			else if(chosenAction == BankActions.Authorization){
	        				addAuthorizationInUI((int)client.getClientId(), textField1.getText(), serviceGiver);
	        			}
	        			else if(chosenAction == BankActions.BankCharge){
	        				bankChargeInUI((int)client.getClientId(), Double.parseDouble(textField1.getText()));
	        			}
	        			else if(chosenAction == BankActions.BankCredit){
	        				bankCreditInUI((int)client.getClientId(), Double.parseDouble(textField1.getText()));
	        			}
	                	primaryStage.close();
	                }
	            }
